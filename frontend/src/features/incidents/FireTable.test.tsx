...
describe('FireTable', () => {
  ...
  it('renders every detection from the current map viewport', () => {
    render(<FireTable liveFires={makeLiveFires([strongFire, weakFire])} onSelectFire={vi.fn()} />);
  ...
  it('filters rows by a resolved country from the main search box', async () => {
    ...
    render(<FireTable liveFires={makeLiveFires([strongFire, otherCountryFire])} onSelectFire={vi.fn()} />);
  ...
  it('sorts by intensity when the column header is clicked', () => {
    render(<FireTable liveFires={makeLiveFires([weakFire, strongFire])} onSelectFire={vi.fn()} />);
  ...
  it('calls onSelectFire when "Ver en mapa" is clicked', () => {
    ...
    render(<FireTable liveFires={makeLiveFires([strongFire])} onSelectFire={onSelectFire} />);
  ...
  it('shows an empty state when no fires match the active filters', () => {
    render(<FireTable liveFires={makeLiveFires([weakFire])} onSelectFire={vi.fn()} />);
    fireEvent.click(screen.getByRole('button', { name: /Avanzado/ }));
    fireEvent.change(screen.getByLabelText('FRP mínimo en MW'), { target: { value: '25' } });
  ...
  it('prompts to load data before any viewport has been fetched', () => {
    render(<FireTable liveFires={makeLiveFires([]) } onSelectFire={vi.fn()} />);
    // no detections yet
