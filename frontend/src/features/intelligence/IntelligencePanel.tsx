...
export function IntelligencePanel({ liveFires, fireSpread, fireReport, reverseLocation, selectedFire, onClose }: {
  liveFires: LiveFires; fireSpread: FireSpread; fireReport: FireReport;
  reverseLocation?: AsyncState<ReverseLocationResponse>; selectedFire: FireDetection | null; onClose: () => void;
}) {
  return (
    <aside className="intelligence" aria-label="Inteligencia operativa">
      ...
      <LiveFirePanel liveFires={liveFires} fireSpread={fireSpread} fireReport={fireReport} reverseLocation={reverseLocation} selectedFire={selectedFire} />
    </aside>
  );
}
