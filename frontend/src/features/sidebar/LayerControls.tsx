...
const layerItems: Array<{ id: LayerKey; label: string; icon: typeof Flame }> = [
  ...
  { id: 'camera', label: 'Fuegos detectados por cámara', icon: Camera },
];

export function LayerControls({ layers, onToggle }: { layers: Record<LayerKey, boolean>; onToggle: (layer: LayerKey) => void }) {
  return (
    <section className="panel-section">
      <div className="section-heading"><span className="eyebrow">Capas operativas</span><span>{Object.values(layers).filter(Boolean).length}/3</span></div>
